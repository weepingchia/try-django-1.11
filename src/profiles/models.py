...
class ProfileManager(models.Manager):
	def toggle_follow(self, request_user, username_to_toggle):
		...
		user = request_user
		is_following = False
		# toggle user with follow or unfollow
...
class Profile(models.Model):
	...
	def send_activation_email(self):
		...
		if not self.activated:
			...
			html_message = '<h1>This is an register HTML test</h1>'
			print(message)
			# send_mail = send_mail(
			# 				subject, 
			# 				message, 
...
def post_save_user_receiver(sender, instance, created, *args, **kwargs):
	if created:
		...
		default_user_profile = Profile.objects.get_or_create(user__id=1)[0] # user__username = follower
		default_user_profile.followers.add(instance) # who is following me
		profile.followers.add(default_user_profile.user) # who im following
